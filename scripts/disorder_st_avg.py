...
# ----------------------------------------------------------
df_cleared["mean-z-score"] = df_cleared["z-score"].apply(
    lambda x: np.mean(np.array(x)[np.where(np.array(x) != 999)])
)
df_cleared["std-z-score"] = df_cleared["z-score"].apply(
    lambda x: np.std(np.array(x)[np.where(np.array(x) != 999)])
)
...
mean_z_scores_buckets = [
    np.quantile(np.array(df_cleared["mean-z-score"]), q) for q in [0.25, 0.5, 0.75]
]
df_117["mean-z-score"] = df_117["zscore"].apply(
    lambda x: np.mean(np.array(x)[np.where(np.array(x) != 999)])
)
...
def get_embeddings(
    ...
    for k in range(len(indexes)):
        ...
        repr_esm = (
            outputs.last_hidden_state[0, 1 : 1 + len(seq), ...].clone().detach().cpu().numpy()
        )
...
for _idx, _seed in enumerate(seed_list):
    ...
    train_df_sample = pd.concat(
        [
            df_cleared[df_cleared[category_col] == k].sample(num_training)
            for k in df_cleared[category_col].unique()
        ]
    )
    ...
    for model_type in ["esm-st"]:
        ex_train[model_type], zed_train[model_type] = get_embeddings(
        ...
        ex_test[model_type], zed_test[model_type] = get_embeddings(
            list(df_117["brmid"]),
            df_117,
            test_st_repr,
            esm_model_8bit,
    ...
    r2_score_skl = metrics.r2_score(y_test, predict_skl)
    print("r2 with skl:", r2_score_skl)
    print(
        f"MAE/std with skl - {metrics.mean_absolute_error(y_test, predict_skl):.4f} /"
        f" {np.std(abs(y_test - predict_skl)):.4f}"
    )
    # correlation
    print(
        f"Number: {_idx}; Seed: {_seed} ElasticNet - Correlation between the predicted and the"
        " ground         truth on the test set: "
        f" {scipy.stats.spearmanr(y_test, predict_skl).correlation:.4f}"
    )
    print("---------------------------------")
    ...
    res[_seed] = {
        "seed": _seed,
        "r2": r2_score_skl,
        "MAE": metrics.mean_absolute_error(y_test, predict_skl),
