...
from sklearn.model_selection import KFold, StratifiedKFold
from sklearn.metrics import r2_score, roc_auc_score, f1_score, precision_recall_fscore_support, average_precision_score, precision_recall_curve, roc_curve
import sklearn
from sklearn.neighbors import KNeighborsClassifier
...
def z_score_bucketing(df_row, std_cutoff=4.046, mean_cutoffs=[12.55, 10.51]):
    '''
    The numbers used for cutoffs here come from 50% quantiles of the
    relevant quantities, i.e. mean and std
    '''
...
def get_embeddings(indexes: List[str], df, st_repr: List, esm_model_8bit, esm_tokenizer, z_col='z-score', drop_missing=True, add_st_repr=True) -> Tuple:
    '''
...
for _idx, _seed in enumerate(seed_list):
    ...
    train_df_sample = pd.concat([df_cleared[df_cleared[category_col]==k].sample(num_training) for k in df_cleared[category_col].unique()])
    ...
    print(f"MAE/std with skl - {sklearn.metrics.mean_absolute_error(y_test, predict_skl):.4f} / {np.std(abs(y_test-predict_skl)):.4f}")
    # correlation
    ...
    clf = SGDClassifier(loss='log_loss', max_iter=500000, tol=0.0005, penalty='elasticnet', alpha=0.0001, random_state=72)
