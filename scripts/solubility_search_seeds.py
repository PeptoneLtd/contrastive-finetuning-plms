...
def main():
    ...
    for partition in range(5):  # df_psi['Partition'].unique():
        ...
        fold_test_iterator = list(zip(df_psi[df_psi['Partition'] == partition][seq_col],
                                      df_psi[df_psi['Partition'] == partition][category_col]))
        test_seqs, y_test = list(zip(*fold_test_iterator))

        fold_train_iterator = list(zip(df_psi[df_psi['Partition'] != partition][seq_col],
                                       df_psi[df_psi['Partition'] != partition][category_col]))
        ...
        train_df_sample = pd.concat(
            [df_sub[df_sub[category_col] == k].sample(num_training) for k in df_sub[category_col].unique()])
        ...
        try:
            with ClearCache():
                st_model.fit(train_objectives=[(train_dataloader, train_loss)], epochs=num_epochs,
                             warmup_steps=warmup_steps, show_progress_bar=True, use_amp=True)
        except:
            continue
        ...
        clf = sklearn.linear_model.SGDClassifier(loss='log_loss', max_iter=500000, tol=0.0005, penalty='elasticnet',
                                                 alpha=0.0001, random_state=72)
        ...
        res_fold[partition]['fold']['precision'].append(
            metrics.average_precision_score(y_test, clf.predict_proba(X_test)[:, 1], average='weighted'))
        ...
        res_fold[partition]['val_fold']['precision'].append(
            metrics.average_precision_score(y_val, clf.predict_proba(X_val)[:, 1], average='weighted'))
