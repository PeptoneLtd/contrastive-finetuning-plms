...
import transformers
from sentence_transformers import SentenceTransformer, SentencesDataset, InputExample, losses, models, datasets, evaluation, util
from torch.utils.data import DataLoader
...
# ---------------------------------------------------------------------------------------------------------------------------
esm_model_tag = 'facebook/esm2_t30_150M_UR50D' # or to get the small model choose replace this with 'facebook/esm2_t12_35M_UR50D'
...
if esm_model_tag=='facebook/esm2_t30_150M_UR50D':
    seed_list = np.random.choice(10000, 5) # generate random seeds, one for each partition in the 5 fold CV
    BATCH_SIZE = 6
else:
    seed_list = np.random.choice(10000, 5) # generate random seeds, one for each partition in the 5 fold CV
    BATCH_SIZE = 16
...
def sequence_pairs_generation(dataframe: DataFrame, number_of_sentences_per_sequence: int = 5) -> list:
    ...
    for i in range(number_of_sentences_per_sequence):
        for sequence_index in range(len(sequences)):
            ...
            concord_sequence_index = np.random.choice(bucket_indexes[np.where(numClassesList == label)[0][0]])
...
def main():
    ...
    res_fold = {i: {'fold':copy.deepcopy(empty_metric_dict),
                    'val_fold': copy.deepcopy(empty_metric_dict)}
    ...
    for partition in range(5): #df_psi['Partition'].unique():
        ...
        fold_test_iterator = list(zip(df_psi[df_psi['Partition']==partition][seq_col], df_psi[df_psi['Partition']==partition][category_col]))
        test_seqs, y_test = list(zip(*fold_test_iterator))

        fold_train_iterator = list(zip(df_psi[df_psi['Partition']!=partition][seq_col], df_psi[df_psi['Partition']!=partition][category_col]))
        train_seqs, y_train = list(zip(*fold_train_iterator))

        set_seed(_seed)


        train_df_sample = pd.concat([df_sub[df_sub[category_col]==k].sample(num_training) for k in df_sub[category_col].unique()])
        ...
        try:
            with ClearCache():
                st_model.fit(train_objectives=[(train_dataloader, train_loss)], epochs=num_epochs, warmup_steps=warmup_steps, show_progress_bar=True, use_amp=True)
        except:
            continue
        ...
        clf = sklearn.linear_model.SGDClassifier(loss='log_loss', max_iter=500000, tol=0.0005, penalty='elasticnet', alpha=0.0001, random_state=72)
        ...
        print(metrics.average_precision_score(y_test, clf.predict_proba(X_test)[:, 1], average='micro'))
        ...
        res_fold[partition]['fold']['APS'].append(metrics.average_precision_score(y_test, clf.decision_function(X_test)))
        res_fold[partition]['fold']['acc'].append(clf.score(X_test, y_test))
        res_fold[partition]['fold']['precision'].append(metrics.average_precision_score(y_test, clf.predict_proba(X_test)[:, 1], average='weighted'))
        res_fold[partition]['fold']['recall'].append(metrics.recall_score(y_test, clf.predict(X_test), average='weighted'))
        res_fold[partition]['fold']['auc_roc'].append(metrics.roc_auc_score(y_test, clf.predict_proba(X_test)[:, 1]))
        ...
        print(metrics.average_precision_score(y_val, clf.predict_proba(X_val)[:, 1], average='micro'))
        ...
        res_fold[partition]['val_fold']['APS'].append(metrics.average_precision_score(y_val, clf.decision_function(X_val)))
        res_fold[partition]['val_fold']['acc'].append(clf.score(X_val, y_val))
        res_fold[partition]['val_fold']['precision'].append(metrics.average_precision_score(y_val, clf.predict_proba(X_val)[:, 1], average='weighted'))
        res_fold[partition]['val_fold']['recall'].append(metrics.recall_score(y_val, clf.predict(X_val), average='weighted'))
        res_fold[partition]['val_fold']['auc_roc'].append(metrics.roc_auc_score(y_val, clf.predict_proba(X_val)[:, 1]))
